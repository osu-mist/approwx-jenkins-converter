...
with open(args.file, 'r') as f:
    ...
    for line in f:
        ...
        if line.startswith('START=so_job_table'):
            ...
            exp_dict['so_job_table'] = {'so_module': so_module, 'params': []}
        ...
        if line.startswith('START=so_job_prompts'):
            ...
            exp_dict['so_job_prompts'].append({
                'so_module': so_module,
                'so_prompt': so_prompt,
                'params': []
            })
        ...
        if line.startswith('START=so_object_cond'):
            ...
            exp_dict['so_object_cond'].append({
                'so_module': so_module,
        ...
        if match and is_job_table:
            exp_dict['so_job_table']['params'].append({
                match.group(1): match.group(2)
            })
        elif match and is_job_prompts:
            exp_dict['so_job_prompts'][int(so_prompt)-1]['params'].append({
                match.group(1): match.group(2)
            })
        elif match and is_object_cond:
            exp_dict['so_object_cond'][int(so_soc_order)-1]['params'].append({
                match.group(1): match.group(2)
            })

        # TODO: need to handle line starts with 'DELETE'?

exp_dict['checks'] = check_dict
